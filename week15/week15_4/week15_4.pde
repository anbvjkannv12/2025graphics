...
// 修改自 week15_03_multiple_windows_PGraphics_pg_pg2_pg3_pg4
// 偷 Arcball 從 File > Examples > Demos > Tests > multiple_windows 
PGraphics pg, pg2, pg3, pg4; // 在外面宣告，不同人都可以用
Arcball arcball; // 用偷來的 Arcball 宣告小寫的 arcball
void setup(){
  size(400, 400, P3D); // 主要的視窗
  arcball = new Arcball(this, 200);
  pg = createGraphics(200, 200, P3D); // 有一個小視窗
  pg2 = createGraphics(200, 200, P3D); // 有一個小視窗
  pg3 = createGraphics(200, 200, P3D); // 有一個小視窗
  pg4 = createGraphics(200, 200, P3D); // 有一個小視窗
}
void mousePressed(){
...
void draw(){
  ...
  pg2.endDraw();
  pg3.beginDraw();
  pg3.background(255, 0, 255); // 紫色小背景
  pg3.translate(100, 100);
  pg3.rotateY(radians(frameCount));
  pg3.box(100);
  pg3.endDraw();
  pg4.beginDraw();
  pg4.background(0, 0, 255); // 紅色小背景
